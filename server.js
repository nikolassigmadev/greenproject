...
// =====================================================
// MISSING PRODUCTS — enrichment queue
// =====================================================

const MISSING_PRODUCTS_FILE = join(__dirname, 'data', 'missing-products.json');

function loadMissingProducts() {
  try {
    if (existsSync(MISSING_PRODUCTS_FILE)) {
      return JSON.parse(readFileSync(MISSING_PRODUCTS_FILE, 'utf-8'));
    }
  } catch (e) {
    console.warn('Failed to load missing products:', e.message);
  }
  return [];
}

function saveMissingProducts(products) {
  const dir = dirname(MISSING_PRODUCTS_FILE);
  if (!existsSync(dir)) mkdirSync(dir, { recursive: true });
  writeFileSync(MISSING_PRODUCTS_FILE, JSON.stringify(products, null, 2));
}

/**
 * POST /api/missing-products
 * Submit a product that wasn't found in OpenFoodFacts.
 * Creates an empty entry in the enrichment queue for later data gathering.
 */
app.post('/api/missing-products', async (req, res) => {
  try {
    const { productName, submittedAt, source, openAiBrand, openAiProduct, offTopResult } = req.body;
    if (!productName || typeof productName !== 'string' || productName.trim().length < 2) {
      return res.status(400).json({ success: false, error: 'productName is required (min 2 chars)' });
    }

    const products = loadMissingProducts();

    // Deduplicate: skip if same product name already exists (case-insensitive)
    const normalized = productName.trim().toLowerCase();
    const existing = products.find(p => p.productName.toLowerCase() === normalized);
    if (existing) {
      console.log(`⏩ Duplicate missing product skipped: "${productName}" (existing id: ${existing.id})`);
      return res.json({ success: true, id: existing.id, productName: existing.productName, duplicate: true });
    }
    ...
    const entry = {
      ...
      openAiBrand: openAiBrand || null,        // what OpenAI identified as the brand
      openAiProduct: openAiProduct || null,    // what OpenAI identified as the product
      offTopResult: offTopResult || null,      // what OFF returned as its best match
      status: 'pending',       // pending → enriching → done
      enrichedData: null,      // filled by agent later
    };
    products.push(entry);
    saveMissingProducts(products);

    console.log(`📝 Missing product submitted: "${entry.productName}" (${entry.id})`);

    // Fire-and-forget: ask OpenAI to gather data about this product
    if (openaiClient) {
      gatherProductData(entry.id, entry.productName).catch(err =>
        console.error(`Agent enrichment failed for "${entry.productName}":`, err.message)
      );
    }

    res.json({ success: true, id: entry.id, productName: entry.productName });
  } catch (error) {
    console.error('Missing product submission error:', error);
    res.status(500).json({ success: false, error: 'Failed to submit missing product' });
  }
});

/**
 * GET /api/missing-products
 * List all missing products (admin use).
 */
...
/**
 * Agent: gather ethical data about a missing product via OpenAI.
 * Updates the entry in-place once data is collected.
 */
async function gatherProductData(entryId, productName) {
  console.log(`🤖 Agent gathering data for: "${productName}"...`);
  ...
  const completion = await openaiClient.chat.completions.create({
    ...
    messages: [
      {
        role: 'system',
        content: `You are an expert ethical-shopping analyst. Given a product name, research and return a JSON object with everything you know about its ethical profile. Be honest about uncertainty.
...
Return ONLY valid JSON:
{
  ...
  "category": "string (e.g. Snacks, Beverages, Dairy)",
  "summary": "1-2 sentence ethical verdict",
  "overallScore": "A | B | C | D | E",
  "environment": { "score": "A-E or unknown", "notes": "string" },
  "nutrition": { "score": "A-E or unknown", "novaGroup": 1-4 or null, "notes": "string" },
  "labor": { "risk": "low | medium | high | critical | unknown", "notes": "string" },
  "animalWelfare": { "risk": "low | medium | high | critical | not-applicable", "notes": "string" },
  ...
  "disclaimer": "string"
      ...
      {
        role: 'user',
        content: `Analyze this product for ethical shopping: "${productName}"`
      },
    ],
  });
  ...
  // Update the entry
  const products = loadMissingProducts();
  const idx = products.findIndex(p => p.id === entryId);
  if (idx >= 0) {
    products[idx].status = parsed ? 'done' : 'failed';
    products[idx].enrichedData = parsed;
    products[idx].enrichedAt = new Date().toISOString();
    saveMissingProducts(products);
    console.log(`✅ Agent enriched "${productName}": ${parsed ? 'success' : 'failed to parse'}`);
  }
}
